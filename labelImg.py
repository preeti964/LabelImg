...
class MainWindow(QMainWindow, WindowMixin):
    ...
    def __init__(self, defaultFilename=None, defaultPrefdefClassFile=None):
        ...
        self.canvas.newShape_rectbox.connect(self.newShape_rectbox)
        self.canvas.newShape_polygon.connect(self.newShape_polygon)
        self.canvas.newShape_ellipse.connect(self.newShape_ellipse)
        ...
        save_polygon = action('&Save\nPolygon', self.saveFile_polygon,
                              'Ctrl+Alt+S', 'save-polygon', u'Save labels to img', enabled=False)
        ...
        create_polygon = action('Create\nPolygon', self.createShape_polygon,
                                'p', 'new', u'Draw a new Polygon', enabled=False)

        create_ellipse = action('Create\nEllipse', self.createShape_ellipse,
                                'p', 'new', u'Draw a new Polygon', enabled=False)

        delete = action('Delete', self.deleteSelectedShape,
                        'Delete', 'delete', u'Delete', enabled=False)

        # delete_polygon = action('Delete\nPolygon', self.poly_deleteSelectedShape,
        #                 'Delete', 'delete', u'Delete', enabled=False)

        copy = action('&Duplicate', self.copySelectedShape,
        ...
        self.actions = struct(save=save, save_polygon=save_polygon, saveAs=saveAs, open=open, close=close,
                              lineColor=color1, fillColor=color2,
                              create=create, create_polygon=create_polygon, create_ellipse=create_ellipse, delete=delete, edit=edit, copy=copy,
                              ...
                              fileMenuActions=(
                                  open, opendir, save, save_polygon, saveAs, close, quit),
                              beginner=(), advanced=(),
                              editMenu=(edit, copy, delete,
                                        None, color1, color2),
                              beginnerContext=(create, create_polygon, create_ellipse, edit, copy, delete,),
                              advancedContext=(createMode, editMode, edit, copy,
                                               delete, shapeLineColor, shapeFillColor),
                              onLoadActive=(
                                  close, create, create_polygon, create_ellipse, createMode, editMode),
        ...
        addActions(self.menus.file,
                   (open, opendir, changeSavedir, openAnnotation, self.menus.recentFiles, save, save_polygon, saveAs, close, None, quit))
        ...
        self.actions.beginner = (
            open, opendir, changeSavedir, openNextImg, openPrevImg, verify, endSegmentation, save, save_polygon, None, create, create_polygon, create_ellipse,
            copy, delete, None,
            zoomIn, zoom, zoomOut, fitWindow, fitWidth)

        self.actions.advanced = (
            open, opendir, changeSavedir, openNextImg, openPrevImg, endSegmentation, save, save_polygon, None,
            createMode, create_polygon, editMode, None,
            hideAll, showAll)
    ...
    def toggleAdvancedMode(self, value=True):
        ...
        if self.PolygonType == 'draw_rectbox':
            self.canvas.setEditing(True, 'draw_rectbox')
        else:
            self.canvas.setEditing_polygon(True, 'draw_polygon')
        self.populateModeActions()
    ...
    def populateModeActions(self):
        ...
        actions = (self.actions.create, self.actions.create_polygon, self.actions.create_ellipse) if self.beginner() \
    ...
    def setDirty_polygon(self):
        self.dirty = True
        self.actions.save_polygon.setEnabled(True)
    ...
    def createShape(self):
        ...
        self.canvas.setEditing(False, 'draw_rectbox')
        self.actions.create.setEnabled(False)

    def createShape_polygon(self):
        assert self.beginner()
        self.canvas.setEditing_polygon(False, 'draw_polygon', 'polygon')
        self.actions.create.setEnabled(False)

    def createShape_ellipse(self):
        assert self.beginner()
        self.canvas.setEditing_polygon(False, 'draw_polygon', 'ellipse')
        self.actions.create.setEnabled(False)
    ...
    def newShape_polygon(self):
        ...
        if text is not None:
            ...
            if self.beginner():  # Switch to edit mode.
                ...
                self.canvas.setEditing_polygon(True, 'draw_polygon', 'polygon')
                self.actions.create.setEnabled(True)
            else:
                self.actions.editMode.setEnabled(True)
            self.setDirty_polygon()
    ...
    def newShape_ellipse(self):
        ...
        if text is not None:
            ...
            if self.beginner():  # Switch to edit mode.
                ...
                self.canvas.setEditing_polygon(True, 'draw_polygon', 'ellipse')
                self.actions.create.setEnabled(True)
            else:
                self.actions.editMode.setEnabled(True)
            self.setDirty_polygon()
        ...
        if unicodeFilePath and os.path.exists(unicodeFilePath):
            ...
            if self.usingPascalVocFormat is True:
                if self.defaultSaveDir is not None:
                    ...
                    segPath = os.path.splitext(filePath)[0] + '.png'
                    if os.path.isfile(segPath):
    ...
    def openDir(self, _value=False):
        ...
        dirpath = ustr(QFileDialog.getExistingDirectory(self, '%s - Open Directory' % __appname__, path, QFileDialog.ShowDirsOnly | QFileDialog.DontResolveSymlinks))
        if dirpath is not None and len(dirpath) > 1:
    ...
    def saveFile(self, _value=False):
        if self.defaultSaveDir is not None and len(ustr(self.defaultSaveDir)):
            if self.filePath:
                ...
                segFilename = os.path.splitext(imgFileName)[0] + '.png'
    ...
    def saveFile_polygon(self, _value=False):
        if self.defaultSaveDir is not None and len(ustr(self.defaultSaveDir)):
            if self.filePath:
                imgFileName = os.path.basename(self.filePath)
                savedFileName = os.path.splitext(imgFileName)[0] + XML_EXT
