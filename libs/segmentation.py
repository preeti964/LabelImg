...
class Segmentation(object):
    ...
    def save_seg(self, save_path, category_path=None):
        ...
        img_pil = Image.fromarray(base, 'L')
    ...
    def create_mask(self, rect=None, init_with_rect=False):
        ...
        segment = np.where((self._mask == 0), 0, 255).astype(np.uint8)
        contours, hir = cv2.findContours(segment, cv2.RETR_CCOMP, cv2.CHAIN_APPROX_NONE)
    ...
    def create_poly_mask(self, rect=None):
        """Creates/Updates currently working segment with a newly drawn mask"""
        base = np.zeros(self._img.shape[:2], dtype=np.uint8)
        img_conutour = cv2.fillPoly(base, pts=rect, color=(255))
        contours, hir = cv2.findContours(img_conutour, cv2.RETR_CCOMP, cv2.CHAIN_APPROX_NONE)
        if len(contours):
            current_contours = []
            for contour in contours:
                if cv2.arcLength(contour, False) > self._size_thres or cv2.contourArea(contour) > self._size_thres:
    ...
    def create_cir_mask(self, x, y, r):
        """Creates/Updates currently working segment with a newly drawn mask"""
        base = np.zeros(self._img.shape[:2], dtype=np.uint8)
        cv2.circle(base, (x, y), r, 1)
        contours, hir = cv2.findContours(base, cv2.RETR_CCOMP, cv2.CHAIN_APPROX_NONE)
        res = contours[-2]
        cir_base = np.zeros(self._img.shape[:2], dtype=np.uint8)
        img_contour = cv2.fillPoly(cir_base, pts=[res], color=(255))
        contours, hir = cv2.findContours(img_contour, cv2.RETR_CCOMP, cv2.CHAIN_APPROX_NONE)
        if len(contours):
            current_contours = []
            for contour in contours:
                if cv2.arcLength(contour, False) > self._size_thres or cv2.contourArea(contour) > self._size_thres:
    ...
    def create_ellipse_mask(self, elps_shape):
        """Creates/Updates currently working segment with a newly drawn mask"""
        base = Image.fromarray(np.zeros(self._img.shape[:2], dtype=np.uint8))
        img = ImageDraw.Draw(base)
        img.ellipse(elps_shape, fill=(255))
        nu_base = np.asarray(base)
        contours, hir = cv2.findContours(nu_base, cv2.RETR_CCOMP, cv2.CHAIN_APPROX_NONE)
        if len(contours):
            current_contours = []
            for contour in contours:
                if cv2.arcLength(contour, False) > self._size_thres or cv2.contourArea(contour) > self._size_thres:
